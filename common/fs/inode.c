...
/// Return the first allocated inode starting at the specified index.
/// Return NULL if no allocated inode could be found.
/// @param itab inode table.
/// @param current_index the index at which to start looking. This variable is not modified by the function.
/// @return the first allocated inode or NULL if none found.
inode_t *inode_get_next(inode_table_t *itab, uint_t current_index) {
	printf("    inode get next : %d\n", current_index);
	for (uint_t i = (uint16_t)current_index; i < itab->inode_count; i++) {
		inode_t *inode = inode_get(itab, i);
		if (inode->number > 0) {  // inode numbers > 0 are in use
			return inode;
		}
